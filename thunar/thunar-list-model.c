/* $Id: thunar-list-model.c 28811 2008-11-15 12:06:38Z jannis $ */
/*-
 ...
 * WB NOV 2010
 *
 * When sorted by type, use file extension instead of MIME
 *
 */

/*
 * take the string after the last . as extension
 */
static const char *
              ...
              gboolean          case_sensitive)
{
	...
	const gchar *a_name = thunar_vfs_path_get_name (thunar_file_get_path (a));
	const gchar *b_name = thunar_vfs_path_get_name (thunar_file_get_path (b));
	const gchar *a_ext = get_file_extension(a_name);
	const gchar *b_ext = get_file_extension(b_name);
	...
	if (G_UNLIKELY (!case_sensitive))
	    result = strcasecmp (a_ext, b_ext);
	else
	    result = strcmp (a_ext, b_ext);
